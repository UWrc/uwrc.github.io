...
The value given for `--ntasks-per-node` should be `40` for newer `klone` nodes if you wish to maximize use of an entire node.
...
All of these man pages can also be viewed on `klone` by running `man <command>`. 
